      ...
      {topics.map((topic) => {
        ...
        return (
          <motion.button
            ...
            whileHover={{ scale: 1.05, y: -2 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => onTopicChange(topic.id)} className="bg-white/10 text-slate-950 px-6 py-3 font-bold opacity-100 rounded-xl flex items-center gap-2 transition-all hover:bg-white/20 border border-white/20">
